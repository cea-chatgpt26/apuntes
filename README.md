# Curso de ChatGPT e Inteligencia Artificial

**60 horas · 20 días · 3h/día**

Cada día: 65 min contenido → 20 min descanso → 65 min contenido → 30 min trabajo en equipo

---

## Módulo 1 — Fundamentos de IA y ChatGPT
*10 horas · Días 1-3*

| Día | Archivo | Estado |
|---|---|---|
| 1 | [¿Qué es la Inteligencia Artificial?](1-que-es-ia.md) | ✅ |
| 2 | [ChatGPT y los Modelos de Lenguaje](2-chatgpt-y-llm.md) | ✅ |
| 3 | [Limitaciones de la IA y tu Primer Prompt](3-limitaciones-y-prompts.md) | ✅ |

---

## Módulo 2 — Desafíos y Avances en IA
*30 horas · Días 4-13*

| Día | Archivo | Estado |
|---|---|---|
| 4 | [Prompt Engineering I: Few-Shot Prompting](4-prompt-engineering-1.md) | ✅ |
| 5 | [Prompt Engineering II: Chain-of-Thought e Iteración](5-prompt-engineering-2.md) | ✅ |
| 6 | Prompts por objetivo y sector | ⬜ |
| 7 | IA generativa: imágenes (DALL·E, Midjourney, Stable Diffusion) | ⬜ |
| 8 | IA generativa: vídeo y audio (Sora, Runway, ElevenLabs, Suno) | ⬜ |
| 9 | IA para código y agentes (Copilot, Cursor, Claude Code) | ⬜ |
| 10 | Desafíos éticos: sesgos, privacidad, derechos de autor | ⬜ |
| 11 | Desafíos sociales: deepfakes, regulación, Ley IA UE | ⬜ |
| 12 | Futuro del trabajo con IA | ⬜ |
| 13 | El ecosistema actual: Perplexity, NotebookLM, Gamma | ⬜ |

---

## Módulo 3 — Aplicaciones Prácticas de ChatGPT
*20 horas · Días 14-20*

| Día | Archivo | Estado |
|---|---|---|
| 14 | Productividad con ChatGPT | ⬜ |
| 15 | Creación de contenido con IA | ⬜ |
| 16 | Make I: primera automatización | ⬜ |
| 17 | Make II: nodo OpenAI y lógica condicional | ⬜ |
| 18 | Make III: flujos avanzados y los 5 flujos del curso | ⬜ |
| 19 | Integración total: ChatGPT + herramientas + Make | ⬜ |
| 20 | Presentación final | ⬜ |

---

## Estructura de cada archivo

Todos los días siguen el mismo patrón:

1. **Recapitulando** — pregunta rápida sobre el día anterior
2. **Contenido** — teoría en bloques cortos con ejemplos
3. **Vídeo** — referencia de YouTube con duración
4. **Ejercicios** — entre 2 y 3, con respuestas desplegables donde aplica
5. **Trabajo en equipo** — 30 min, dinámico y conectado al contenido del día
6. **Lo que aprendiste hoy** — resumen en 5 puntos

---

## Vídeos del curso

| Día | Canal | Vídeo | Duración |
|---|---|---|---|
| 1 | DotCSV | Lo que YA sabes sobre Inteligencia Artificial | ≈17 min |
| 2 | Nate Gentile | ¿Cómo funciona ChatGPT? La revolución de la IA | 28 min |
| 4 | DotCSV | Zero-shot, One-shot y Few-shot Prompting | ≈20 min |
| 5 | — | Prompts avanzados: Chain of Thought y Tree of Thought | ≈12 min |

---

## Trabajos en equipo (30 min/día)

| Día | Nombre | Dinámica |
|---|---|---|
| 1 | ¿IA o no IA? | Clasificar 12 productos por tipo de IA |
| 2 | Battle de temperaturas | Mismo prompt, 4 tonos distintos, comparar |
| 3 | Caza de alucinaciones | Competición para encontrar la alucinación más elaborada |
| 4 | El prompt perfecto | Cada equipo usa una técnica distinta, votación ciega |
| 5 | El prompt en 3 rondas | Mismo problema, iteración progresiva en equipo |
| 6-20 | Por definir | — |
